...
# takes in files with information on gene sequences 
#scores the codons, makes other scores using raw scores 
# creates a dictionary data structure that stores gene subdictionaries that store

# geneName codons  codonScores Gene_Name	Alias	Number_of_reads	Normalized_expression	Median	PolyA_class	25%_percentile	75%_percentile	MAX_polyA	MIN_polyA	RANGE_polyA	Mean	Geometric_mean	Standard_deviation	Fop	Codon_class
...
#For debuging shows the program startec
print('Start')


# Functions
#generates the other scores from the raw scores
...
#find the precentiles of the scores
...
#calculates change in raw scores
...
#calculates the sum of 5 raw scores on each side to the entry
#does not include entries with less than 5 on each side
...
#gets the informtation from sequenceInfor.txt on the genes that we have poly A tail data for
...
#resets scores to 0 to prevent carry over of data from one sequence to another
def emptyScores():
...
#identifies what the codons of the sequence are
